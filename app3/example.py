...
def main():
    ...
    layout = [
        [
            sg.Text(
                font=("Helvetica", 20), text_color=sg.YELLOWS[0], key="-CPU PERCENT-"
            )
        ],
        [
            sg.Text(size=(35, 12), font=("Courier New", 12), key="-PROCESSES-")
        ],  # size will determine how many processes shown
        [
    ...
    try:
        ...
        while True:
            ...
            if event in (sg.WIN_CLOSE_ATTEMPTED_EVENT, "Exit"):
                ...
                )  # save window location before exiting
                break
            if event == "Edit Me":
                ...
                event == "-CPU UPDATE FROM THREAD-"
            ):  # indicates data from the thread has arrived
                ...
                if procs:
                    ...
                    )  # reverse sort to get highest CPU usage on top
                    if top_sorted:
                        top_sorted.pop(0)  # remove the idle process
                    display_string = "\n".join(
                        [f"{cpu/10:2.2f} {proc:23}" for proc, cpu in top_sorted]
                    )
        ...
        sg.Print(
            "Exception = ", e, wait=True
        )  # IMPORTANT to add a wait/blocking so that the print pauses execution. Otherwise program continue and exits
